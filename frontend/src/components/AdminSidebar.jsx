...
const AdminSidebar = () => {
    ...
    const menuItems = [
        { icon: <Home size={24} />, label: 'Overview', path: '/admin' },
        { icon: <Plus size={24} />, label: 'Upload', path: '/admin/upload', center: true },
        ...
        <aside className="fixed left-0 top-0 h-screen w-20 md:w-64 bg-white border-r border-gray-100 pt-24 pb-8 flex flex-col z-40 transition-all duration-300">
            <div className="flex-1 px-4 space-y-2">
                {menuItems.map((item) => (
                    <Link
                        key={item.label}
                        to={item.path}
                        className={`flex items-center gap-4 px-4 py-3 rounded-2xl transition-all duration-200 group ${item.center ? 'bg-red-600 text-white hover:bg-red-700 shadow-lg shadow-red-100 my-4' : (location.pathname === item.path ? 'bg-red-50 text-red-600' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900')}
                            `}
                    >
                        <div className={`${item.center ? 'text-white' : (location.pathname === item.path ? 'text-red-600' : 'text-gray-400 group-hover:text-gray-900')} transition-colors mx-auto md:mx-0`}>
                            {item.icon}
                        </div>
            ...
            {showLogoutModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-sm:w-full max-w-sm p-8 transform transition-all animate-in zoom-in-95 duration-200">
